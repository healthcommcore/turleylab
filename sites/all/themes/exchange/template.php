		...
		if (isset($settings['slider']['layers'])) {
			foreach ($settings['slider']['layers'] as $lid => $layer) {
				if (!empty($layer)) {
					...
					/*
					if ($settings['slider']['config']['controller'] == 'title') {
						$id = $lid + 1;
						$variables['page']['slider']['slider_container']['controller'][$lid] = array(
							'#type' => 'markup',
							'#markup' => "<a class='ls-linkto-{$id}'>{$layer['title']}</a>",
						);
					}
					*/
...
/**
function phptemplate_menu_item($variables){
	print_r($variables);
}
 * Override menu item to allow highlighting of parent menu items
function exchange_menu_item($mid, $children = '', $leaf = TRUE) {
	  return '<li class="'. ($leaf ? 'leaf' : ($children ? 'test' : 'collapsed')) . (menu_in_active_trail($mid) ? ' actual' : '') .'">'. menu_item_link($mid) . $children ."</li>\n";
}
 */
/**
 * Override menu item to allow highlighting of parent menu items
function exchange_menu_item($link, $has_children, $menu='', $in_active_trail = FALSE, $extra_class = NULL){
